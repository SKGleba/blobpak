...
void AES_CBC_decrypt_buffer_with_counter(struct AES_ctx* ctx, uint8_t* countRoundKeys, uint8_t* buf, uint32_t length, uint32_t counter_base);
void AES_CBC_encrypt_buffer_with_counter(struct AES_ctx* ctx, uint8_t* countRoundKeys, uint8_t* buf, uint32_t length, uint32_t counter_base);
...
void aes_cbc_count(uint8_t* key, uint8_t* iv, uint8_t* tweak, uint8_t* data, uint32_t len, int encrypt, uint32_t counter_base) {
    struct AES_ctx aesctx;
    uint8_t countRoundKeys[AES_keyExpSize];
    memset(&aesctx, 0, sizeof(aesctx));
    AES_init_ctx_iv(&aesctx, key, iv);
    KeyExpansion(countRoundKeys, tweak);
    if (encrypt)
        AES_CBC_encrypt_buffer_with_counter(&aesctx, countRoundKeys, data, len, counter_base);
    else
        AES_CBC_decrypt_buffer_with_counter(&aesctx, countRoundKeys, data, len, counter_base);
}
...
// Lets hope i dont introduce any vulnerabilities here
void AES_CBC_encrypt_buffer_with_counter(struct AES_ctx* ctx, uint8_t* countRoundKey, uint8_t* buf, uint32_t length, uint32_t counter_base) {
    uint32_t i;
    uint32_t countBuf[AES_BLOCKLEN / 4];
    uint8_t* Iv = ctx->Iv;
    for (i = 0; i < length; i += AES_BLOCKLEN) {
        countBuf[0] = countBuf[1] = countBuf[2] = countBuf[3] = ((i >> 4) + counter_base);
        Cipher((state_t*)countBuf, countRoundKey);
        XorWithIv(buf, (uint8_t*)countBuf);
        XorWithIv(buf, Iv);
        Cipher((state_t*)buf, ctx->RoundKey);
        Iv = buf;
        buf += AES_BLOCKLEN;
...
void AES_CBC_decrypt_buffer_with_counter(struct AES_ctx* ctx, uint8_t* countRoundKey, uint8_t* buf, uint32_t length, uint32_t counter_base) {
    uint32_t i;
    uint8_t storeNextIv[AES_BLOCKLEN];
    uint32_t countBuf[AES_BLOCKLEN / 4];
    for (i = 0; i < length; i += AES_BLOCKLEN) {
